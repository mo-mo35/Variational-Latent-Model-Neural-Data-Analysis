...
# -------------------------------------------------------------------------
# Baseline Comparison: Direct PCA Approach
# -------------------------------------------------------------------------
...
def compare_pipelines(event_name, data_dict, n_components=6):
    ...
    baseline_metrics = run_pca_baseline(X1_scaled, X2_scaled, n_components)
    ...
    ax.set_title(f'Comparison of Pipelines for {event_name}')
    ax.set_xticks(x)
...
if __name__ == "__main__":
    ...
    # For each event, first compare the pipelines and then run the full pCCA processing
